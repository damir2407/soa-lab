      ...
      <v-navigation-drawer class=EDF2F7 width="100%" permanent>
          ...
          <v-dialog width="500px" v-model="dialog" v-if="modeSwitch === true">
            <template v-slot:activator="{ on, attrs }">
              ...
              <div>
                ...
                <v-btn @click="openWind='OverlayMarinesCountLess'" v-bind="attrs" v-on="on" width="95%"
                       class="navigationButton" style="color: black; border-radius: 1px; margin-top: 40px">
                  Marines-Count меньше ...
                </v-btn>
              </div>
            ...
            <OverlayMarinesCountLess v-if="openWind === 'OverlayMarinesCountLess'" @updateParent="updateDialog"/>
          </v-dialog>
        ...
        <template v-slot:append>
          <v-card-text style="margin-left: 4%; margin-bottom:20px; font-size: 18px">
            Балтабаев Дамир <br> Нуруллаев Даниил <br><br> P34121
          </v-card-text>
        </template>
      </v-navigation-drawer>
    </v-card>
...
import OverlayMarinesCountLess from "@/components/mainPage/elementsOverlays/OverlayMarinesCountLess.vue";

export default {
  name: "LeftNavigationBar",

  components: {
    OverlayMarinesCountLess,
